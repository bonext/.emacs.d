...
**** Other hide general hide functions
...
- ~scroll-margin>0~ is a no-no.  This setting always moves point at least that many lines away from the window boundaries, which, unless you can reliably place point there during the scroll (even in the presence of /jumbo lines/; see below), will cause loop-back.  See #3.
- Virtual Scroll:
